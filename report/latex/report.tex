...
The VESC Tool, a highly versatile and widely used application, was used to configure, and program the Vedder Electronic Speed Controllers (VESCs). Similarly, this tool is used to configure electric skateboards, bikes, robots, and electric vehicle applications. The user-friendly, intuitive interface guides the user through adjusting various parameters. These include customised motor control parameters such as acceleration, braking profiles, maximum motor current, battery voltage limits, throttle curves and so forth. The VESC Tool also provides real-time monitoring information, while logging such data to create a performance record, to display visualizations of relevant parameters. Furthermore, the advanced features include regenerative braking and Field-Orientated Control (FOC). Also known as Vector or Direct Torque Control, FOC allows for precise control of both speed and torque of the motors.  

\subsection{Material and Manufacturing Justifications}
\subsubsection{Extrusion}
Extrusion was used due to its versatility, modularity, and compatibility with t-slot fixings to enable simple yet strong attachment between cross-sections and hardware. The original design used 60x60 extrusion around the payload compartment. After testing, 3060 extrusion was found to be more than adequate to support the weight of the platform and still provided a larger cross-section for rigid attachments. Furthermore, it could sufficiently support the additional payload weight of 80kg, and incorporation of rods and bearings for arm articulation was straightforward.  
\subsubsection{Bearings and Rods}
The selected bearings support steel rods with a diameter of 8mm for significant load distribution. They reduced the friction from the motor drive compartment, allowing for smother and more precise motion. The oval bearings used were adequate for the application, however, to greater increase the robot’s life and reduce stress-wear, bearing with square brackets would provide more support with the additional attachment points. 
\subsubsection{Pulleys and Timing Belts}

\subsubsection{Manufacturing Methods}
...
Both hand saws and bench top saws were used for creating motor driver supports and servo harnesses from scrap wood. They were used to attach the supports directly to extrusion and to replace stand off’s by spreading the force over a larger area. The use of recycled wood was appropriate for the application, meaning the team was happy with its performance.  
...
The \gls{hevcs} is reliably controlled by the user, using methods that enhance its dependability. The purpose behind the prototype is to prove its potential as an alternative method to EV charging, using methods and approaches that the user can rely on and independently control. Using the results of the MECE analysis that define EV owner’s needs to a dependable solution to charging at the convenience, the \gls{hevcs} has proven itself as a viable alternative. Moreover, its creation presents a viable alternative to getting power to the vehicles, provided they are within a certain proximity of the home, that is not limited by obstacles such as kerbs.   
